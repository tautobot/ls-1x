...
directly into ls-1x's in-process file-backed store (``livescore/jsondb.py``). The service
...
Config comes from ``livescore/json_sync/settings.py`` (SYNC_* env vars, with sane
defaults) plus the bare ``JSON_SYNC_SOURCE`` env (default "1x").
"""
import asyncio

from livescore.json_sync.runner import main
